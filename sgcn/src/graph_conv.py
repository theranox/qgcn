...
class SpatialGraphConv(MessagePassing):
    def __init__(self, coors, in_channels, out_channels, hidden_size, dropout=0, use_bias:bool=True):
        """
        ...
        self.lin_in = torch.nn.Linear(coors, hidden_size * in_channels, bias=use_bias)
        self.lin_out = torch.nn.Linear(hidden_size * in_channels, out_channels, bias=use_bias)
...
class SpatialGraphConvEquiv(MessagePassing):
    def __init__(self, coors, in_channels, out_channels, hidden_size, dropout=0, use_bias=False):
        """
        coors - dimension of positional descriptors (e.g. 2 for 2D images)
        in_channels - number of the input channels (node features)
        out_channels - number of the output channels (node features)
        ...
        dropout - dropout rate after the layer
        """
        super(SpatialGraphConvEquiv, self).__init__(aggr='add')
        self.dropout = dropout
        self.lin_in = torch.nn.Linear(coors, hidden_size * in_channels, bias=use_bias)
        self.lin_out = torch.nn.Linear(hidden_size * in_channels, out_channels, bias=use_bias)
        self.relu = torch.nn.ReLU()
        self.dropout = torch.nn.Dropout(self.dropout, inplace=False)
        self.bn1d = torch.nn.BatchNorm1d(out_channels, affine=True)
        self.in_channels = in_channels
    ...
    def message(self, pos_i, pos_j, x_j):
        ...
        relative_pos = pos_j - pos_i  # [n_edges, hidden_size * in_channels]
        spatial_scaling = self.relu(self.lin_in(relative_pos))  # [n_edges, hidden_size * in_channels]
