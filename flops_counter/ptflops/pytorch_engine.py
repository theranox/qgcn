...
def get_flops_pytorch(model, input_res,
    ...
    if isinstance(input_res, type((1,2))): # if only it is tuple
...
def compute_average_flops_cost(self):
    ...
    # NOTE: flops sum seems to double count -> doublecounts conv2d because it appears twice 1. on model itself and 2. inside module list itself.
    # The above is used so that we can enforce same initialization between QGCN and CNN (justify 1.)
    ls = []
    ms = list(self.modules())
    for m in ms:
        try:
            ls.append(m.__flops__)
        except:
            ls.append(0)
    ...
    flops_recount = sum(ls)
    params_sum = get_model_parameters_number(self)
    # x = self.__flops__ if is_supported_instance(self) else 0 
    # print("(flops_recount, params_sum): ", flops_sum, flops_recount, params_sum, x)
    # return flops_sum / self.__batch_counter__, params_sum
    # NOT AWARE OF BATCH SIZE -> USER CAN SCALE BY BATCH-SIZE IF THEY WANT
    return flops_recount, params_sum
...
def batch_counter_hook(module, input, output):
    batch_size = 1
    if len(input) > 0:
        # print(input, len(input), module.__batch_counter__)
